...
def wrapper_trade_action(account_id: str, trade_info_list: list, timeout: int = 1000):
    # 写入扫单文件
    order_added = csv_generate_orders(account_id, trade_info_list)
    if not order_added:  # 一个都没写入，全部撤回
        return {"status": 501, "msg": "failed to append data to order file"}

    # 读取状态变化文件，传递超时参数
    sidlist = order_added.keys()
    status_reports = helper_get_order_status_changes(account_id, sidlist, timeout)
    if not status_reports:  # 文件单输出模式启动失败，文件不存在
        # 已经写入文件单的委托返回“已报”
    ...
    exec_reports = csv_get_exec_report_data(account_id, new_sid_list)
...
def wrapper_cancel_entursts(account_id: str, sid_list: list):
    ...
    sid_added = csv_generate_cancel_orders(account_id, sid_list)
    if not sid_added:
        return {
            "status": 401,
    ...
    # 从状态更新文件中读取撤销结果，字典数据
    status_reports = helper_get_order_status_changes(account_id, sid_added, 1000)
...
def wrapper_get_today_all_entrusts(account_id: str):
    ...
    # 取出所有执行报告中的委托数据
    all_exec_rpts = csv_get_exec_report_data(account_id, None)
